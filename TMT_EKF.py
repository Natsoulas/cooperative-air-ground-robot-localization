...
from src.utils.plotting import compute_nees, compute_nis, plot_filter_performance
...
def run_monte_carlo_simulation(n_runs: int, t_span: tuple) -> Dict:
    ...
    # Initialize storage for both filters
    ...
    for i in range(n_runs):
        ...
        # Compute NEES and NIS values for both filters
...
def plot_monte_carlo_results(results: Dict, alpha: float = 0.05):
    ...
    plot_filter_performance(
        ...
        "LKF (First Monte Carlo Run)"
    )
    
    # Plot NEES results
    plt.figure(figsize=(12, 6))
    plt.subplot(211)
    avg_nees = results['avg_nees']
    chi2_lower = chi2.ppf(alpha/2, df=6) 
    chi2_upper = chi2.ppf(1-alpha/2, df=6)
    
    plt.plot(t, avg_nees, 'b-', label='Average NEES')
    plt.axhline(y=chi2_lower, color='r', linestyle='--', label='Lower bound')
    plt.axhline(y=chi2_upper, color='r', linestyle='--', label='Upper bound')
    plt.grid(True)
    plt.xlabel('Time (s)')
    plt.ylabel('NEES')
    plt.title('Average NEES Over Monte Carlo Runs')
    plt.legend()
    
    # Plot NIS results
    plt.subplot(212)
    avg_nis = results['avg_nis']
    nis_chi2_lower = chi2.ppf(alpha/2, df=5)
    nis_chi2_upper = chi2.ppf(1-alpha/2, df=5)
    
    plt.plot(t, avg_nis, 'b-', label='Average NIS')
    plt.axhline(y=nis_chi2_lower, color='r', linestyle='--', label='Lower bound')
    plt.axhline(y=nis_chi2_upper, color='r', linestyle='--', label='Upper bound')
    plt.grid(True)
    plt.xlabel('Time (s)')
    plt.ylabel('NIS')
    plt.title('Average NIS Over Monte Carlo Runs')
    plt.legend()
...
if __name__ == "__main__":
    ...
    # Perform hypothesis tests on average values
    print("\nPerforming hypothesis tests...")
    
    # NEES test
    nees_results = perform_nees_hypothesis_test(results['avg_nees'])
    print("\nNEES Test Results:")
    ...
    # NIS test
    nis_results = perform_nis_hypothesis_test(results['avg_nis'])
    print("\nNIS Test Results:")
